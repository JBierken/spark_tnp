...
def get_args():
    ...
    parser.add_argument(    '--TrackerOnly',    dest='TrackerOnly', default=False,        action='store_true')
...
def main():
    ...
    if args.TrackerOnly:    seed = 'Tracker_only'
    else:                   seed = 'All_tracks'

    # Get list of all file paths in sub folde 
    file_path               = r'/eos/user/j/jbierken/TnP_ntuples/{Seed}/{part}/{res}/Run{year}/{tier}/*/*/*'.format(
                                Seed    = seed,
                                part    = args.particle, 
                                res     = args.resonance, 
                                year    = args.year, 
